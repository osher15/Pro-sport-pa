...
const FIT=(function(){
  /* ---------- exercise library ---------- */
  /* ---------- מנוע איורים (ריג מפרקים) ---------- */
  const STAND={hd:[50,20],nk:[50,30],hip:[50,54],
    shL:[42,32],elL:[38,45],haL:[35,58],
    shR:[58,32],elR:[62,45],haR:[65,58],
    knL:[45,71],ftL:[43,90],
    knR:[55,71],ftR:[57,90]};
  const P=over=>Object.assign({},STAND,over);

  function human(frames,dur){
    dur=dur||1.4;
    const seg2=(a,b)=>`M${a[0]} ${a[1]} L${b[0]} ${b[1]}`;
    const seg3=(a,b,c)=>`M${a[0]} ${a[1]} L${b[0]} ${b[1]} L${c[0]} ${c[1]}`;
    const anim=(attr,vals)=>`<animate attributeName="${attr}" values="${vals.join(";")}" dur="${dur}s" repeatCount="indefinite"/>`;
    const torso=frames.map(f=>seg2(f.nk,f.hip));
    const armL=frames.map(f=>seg3(f.shL,f.elL,f.haL));
    const armR=frames.map(f=>seg3(f.shR,f.elR,f.haR));
    const legL=frames.map(f=>seg3(f.hip,f.knL,f.ftL));
    const legR=frames.map(f=>seg3(f.hip,f.knR,f.ftR));
    const hx=frames.map(f=>f.hd[0]), hy=frames.map(f=>f.hd[1]);
    const hlx=hx.map(x=>x-3), hly=hy.map(y=>y-3);
    return `<svg viewBox="0 0 100 100" class="fig2">`+
      `<ellipse class="gnd" cx="50" cy="96" rx="18" ry="3"/>`+
      `<path class="limb" d="${armL[0]}">${anim("d",armL)}</path>`+
      `<path class="limb" d="${armR[0]}">${anim("d",armR)}</path>`+
      `<path class="limb" d="${legL[0]}">${anim("d",legL)}</path>`+
      `<path class="limb" d="${legR[0]}">${anim("d",legR)}</path>`+
      `<path class="torso" d="${torso[0]}">${anim("d",torso)}</path>`+
      `<circle class="hd" cx="${hx[0]}" cy="${hy[0]}" r="9">${anim("cx",hx)}${anim("cy",hy)}</circle>`+
      `<circle class="hl" cx="${hlx[0]}" cy="${hly[0]}" r="2.4">${anim("cx",hlx)}${anim("cy",hly)}</circle>`+
      `</svg>`;
  }
  function rig(base,active,dur){ return human([base,active,base],dur); }

  const POSES={
    squat:[STAND, P({hip:[50,68],nk:[50,37],shL:[43,39],elL:[36,44],haL:[32,50],shR:[57,39],elR:[64,44],haR:[68,50],knL:[38,65],ftL:[36,90],knR:[62,65],ftR:[64,90]})],
    lunge:[STAND, P({hip:[48,60],nk:[48,40],shL:[41,42],elL:[35,50],haL:[32,58],shR:[55,42],elR:[61,50],haR:[64,58],knL:[35,66],ftL:[30,90],knR:[62,76],ftR:[70,92]})],
    stepup:[STAND, P({hip:[52,50],nk:[52,30],knL:[40,52],ftL:[38,32],knR:[60,66],ftR:[64,90],shL:[45,32],elL:[38,26],haL:[34,18],shR:[59,32],elR:[64,42],haR:[68,52]})],
    calf:[STAND, P({hd:[50,17],nk:[50,27],hip:[50,51],knL:[45,68],knR:[55,68]})],
    deepsquat:[STAND, P({hip:[50,72],nk:[50,44],shL:[44,46],elL:[36,50],haL:[32,54],shR:[56,46],elR:[64,50],haR:[68,54],knL:[36,68],ftL:[36,90],knR:[64,68],ftR:[64,90]})],
    hinge:[STAND, P({nk:[46,36],hip:[55,48],shL:[40,38],elL:[38,52],haL:[38,66],shR:[52,38],elR:[50,52],haR:[50,66],knL:[46,68],ftL:[43,89],knR:[60,68],ftR:[62,89]})],
    kbswing:[P({nk:[46,36],hip:[55,50],shL:[40,38],elL:[40,52],haL:[42,64],shR:[52,38],elR:[50,52],haR:[50,64],knL:[46,68],ftL:[43,89],knR:[60,68],ftR:[62,89]}),
              P({shL:[42,32],elL:[38,18],haL:[36,6],shR:[58,32],elR:[62,18],haR:[64,6]})],
    row:[P({nk:[46,36],hip:[54,50],shL:[40,38],elL:[36,52],haL:[34,68],shR:[52,38],elR:[48,52],haR:[46,68],knL:[46,68],ftL:[44,89],knR:[58,68],ftR:[60,89]}),
         P({nk:[46,36],hip:[54,50],shL:[40,38],elL:[42,44],haL:[48,40],shR:[52,38],elR:[54,44],haR:[60,40],knL:[46,68],ftL:[44,89],knR:[58,68],ftR:[60,89]})],
    press:[P({shL:[42,32],elL:[36,38],haL:[34,44],shR:[58,32],elR:[64,38],haR:[66,44]}),
           P({shL:[42,32],elL:[38,20],haL:[38,8],shR:[58,32],elR:[62,20],haR:[62,8]})],
    pulldown:[P({shL:[42,32],elL:[36,20],haL:[34,8],shR:[58,32],elR:[64,20],haR:[66,8]}),
              P({shL:[42,32],elL:[34,30],haL:[30,36],shR:[58,32],elR:[66,30],haR:[70,36]})],
    curl:[STAND, P({elL:[38,45],haL:[42,32],elR:[62,45],haR:[58,32]})],
    tripush:[P({elL:[38,40],haL:[36,50],elR:[62,40],haR:[64,50]}), P({elL:[38,40],haL:[38,58],elR:[62,40],haR:[62,58]})],
    carry:[P({knL:[44,68],ftL:[40,90],knR:[56,74],ftR:[64,90]}), P({knL:[56,74],ftL:[64,90],knR:[44,68],ftR:[40,90]})],
    jack:[STAND, P({nk:[50,27],shL:[38,20],elL:[30,10],haL:[26,3],shR:[62,20],elR:[70,10],haR:[74,3],knL:[38,68],ftL:[30,90],knR:[62,68],ftR:[70,90]})],
    knees:[P({hip:[50,52],nk:[50,32],knL:[40,50],ftL:[38,72],knR:[60,72],ftR:[62,90],shL:[43,34],elL:[36,30],haL:[34,22],shR:[57,34],elR:[64,42],haR:[68,52]}),
           P({hip:[50,52],nk:[50,32],knR:[40,50],ftR:[38,72],knL:[60,72],ftL:[62,90],shR:[43,34],elR:[36,30],haR:[34,22],shL:[57,34],elL:[64,42],haL:[68,52]})],
    buttkick:[P({knL:[43,68],ftL:[40,88],knR:[57,70],ftR:[70,74]}), P({knR:[43,68],ftR:[40,88],knL:[57,70],ftL:[30,74]})],
    skater:[P({hip:[46,58],nk:[46,36],knL:[34,66],ftL:[26,88],knR:[58,72],ftR:[68,86],shL:[40,38],elL:[30,44],haL:[24,50],shR:[52,38],elR:[62,30],haR:[70,20]}),
            P({hip:[54,58],nk:[54,36],knR:[66,66],ftR:[74,88],knL:[42,72],ftL:[32,86],shR:[60,38],elR:[70,44],haR:[76,50],shL:[48,38],elL:[38,30],haL:[30,20]})],
      {hd:[22,42],nk:[30,44],hip:[62,44],shL:[32,46],elL:[34,58],haL:[34,70],shR:[32,42],elR:[20,36],haR:[8,32],
       knL:[76,50],ftL:[76,68],knR:[86,40],ftR:[100,30]}
    ],
    catcow:[
      {hd:[22,42],nk:[30,46],hip:[62,44],shL:[32,48],elL:[32,62],haL:[32,74],shR:[32,44],elR:[38,56],haR:[46,60],
       knL:[76,52],ftL:[76,70],knR:[78,46],ftR:[94,54]},
      {hd:[22,36],nk:[30,38],hip:[62,52],shL:[32,40],elL:[32,60],haL:[32,74],shR:[32,36],elR:[38,52],haR:[46,58],
       knL:[76,54],ftL:[76,70],knR:[78,48],ftR:[94,54]}
    ],
    hip90:[
      {hd:[46,30],nk:[48,38],hip:[54,56],shL:[42,40],elL:[36,44],haL:[30,48],shR:[54,40],elR:[60,44],haR:[66,48],
       knL:[36,60],ftL:[30,70],knR:[66,64],ftR:[78,58]},
      {hd:[46,30],nk:[48,38],hip:[54,56],shL:[42,40],elL:[36,44],haL:[30,48],shR:[54,40],elR:[60,44],haR:[66,48],
       knL:[66,64],ftL:[78,58],knR:[36,60],ftR:[30,70]}
    ],
    worldgreat:[
      P({hip:[48,60],nk:[48,40],shL:[41,42],elL:[35,50],haL:[32,58],shR:[55,42],elR:[61,50],haR:[64,58],knL:[35,66],ftL:[30,90],knR:[62,76],ftR:[70,92]}),
      P({hip:[48,60],nk:[46,38],shL:[38,36],elL:[30,20],haL:[24,6],shR:[55,42],elR:[61,50],haR:[64,58],knL:[35,66],ftL:[30,90],knR:[62,76],ftR:[70,92]})
    ],
    thoracic:[
      {hd:[22,42],nk:[30,44],hip:[62,44],shL:[32,46],elL:[34,58],haL:[34,70],shR:[32,42],elR:[36,54],haR:[42,58],
       knL:[76,50],ftL:[76,68],knR:[78,46],ftR:[94,54]},
      {hd:[22,42],nk:[30,44],hip:[62,44],shL:[32,46],elL:[34,58],haL:[34,70],shR:[32,42],elR:[22,28],haR:[14,14],
       knL:[76,50],ftL:[76,68],knR:[78,46],ftR:[94,54]}
    ],
    anklerock:[
      P({hip:[50,60],nk:[50,42],shL:[42,44],elL:[36,50],haL:[32,56],shR:[58,44],elR:[64,50],haR:[68,56],knL:[42,64],ftL:[36,88],knR:[58,64],ftR:[64,88]}),
      P({hip:[50,58],nk:[50,40],shL:[42,42],elL:[36,48],haL:[32,54],shR:[58,42],elR:[64,48],haR:[68,54],knL:[38,64],ftL:[36,88],knR:[62,64],ftR:[64,88]})
    ],
    shouldercar:[
      P({shL:[42,32],elL:[34,26],haL:[28,32],shR:[58,32],elR:[66,26],haR:[72,32]}),
      P({shL:[42,32],elL:[34,40],haL:[30,50],shR:[58,32],elR:[66,40],haR:[70,50]})
    ]
  };
  const FIGS={};
  Object.keys(POSES).forEach(k=>{ FIGS[k]=rig(POSES[k][0],POSES[k][1]); });
  ...
  function renderGymDay(track,d){
    ...
    $("#gym-list").innerHTML=day.ex.map(item=>{
      ...
      return `<div class="fit-station" data-id="${item.id}" style="cursor:pointer">
        <div style="width:42px;height:42px;flex:none">${e?FIGS[e.fig]:""}</div>
