...
/* מאתיים היה מספר שנבחר כדי «להרגיש בטוח», והוא לא היה: המזהה נבנה
   מ-Date.now() ועוד ארבעה תווים אקראיים, ולכן מאתיים מזהים באותה
   מילישנייה מתנגשים בהסתברות של אחוז ומשהו. הבדיקה עברה עשרות
   פעמים ונפלה ב-CI על 199 מתוך 200 — כלומר היא תמיד בדקה את המזל.

   עשרים אלף בלולאה אחת הופכים את זה לוודאי: בקוד הישן זה כשל בכל
   הרצה, ובקוד עם המונה אין התנגשות כלל. */
test("עשרים אלף שיעורים אינם מתנגשים במזהה",()=>{
  const n=20000, ids=new Set();
  for(let i=0;i<n;i++)ids.add(D.newSessionId());
